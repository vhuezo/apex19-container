...
COPY LINUX.X64_180000_db_home.zip apex_19.1_en.zip dbca_silent.sh runsql.sh inst_apex.sql inst_apex2.sql startupdb /tmp/
...
/usr/bin/yum install wget gcc-c++ -y -q
...
#Create oracle .bashrc && Unzip DB software && Install DB software
...
/usr/bin/unzip ${DB_ZIP_ARG} -d ${ORACLE_HOME_ARG} > /dev/null && \
${ORACLE_HOME_ARG}/runInstaller -ignorePrereq -waitforcompletion -silent \
...
USER root
RUN ${ORA_INVENTORY_ARG}/orainstRoot.sh && ${ORACLE_HOME_ARG}/root.sh

USER ${ORACLE_USER_ARG}
#unzip Apex -> create and start Listener -> create DB -> install Apex -> import SUDS schema and workspace -> Create listener.ora -> Create oratab
RUN /usr/bin/unzip ${APEX_ZIP_ARG} -d ${APEX_HOME_ARG} > /dev/null && \
${ORACLE_HOME_ARG}/bin/netca /silent /responsefile ${ORACLE_HOME_ARG}/assistants/netca/netca.rsp && \
